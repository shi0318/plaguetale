...
## 2026-09-07 — 首页问题答案与现有页面集群

- 目标：把已发售后的搜索意图连接到已有的战斗、Story Select、收藏品和成就页面，不新增重复 FAQ 或薄 URL。
- 操作：复核并保留本轮已有的首页、Guide、Walkthrough、Collectibles FAQ 增强；确认这些入口均指向已生成的真实页面，并继续区分官方事实与 Steam 玩家报告。
- 验证：Astro build 生成 54 个页面；SEO 检查和测试均通过；`git diff --check` 通过。
- 边界：没有把社区报告写成本站亲自测试，也没有把发布后的事实扩展为完整章节路线或逐件收藏品地图。
- 下一轮唯一改进：上线后用 GSC 查询和实际页面抓取确认这些答案入口带来的展示词变化。
